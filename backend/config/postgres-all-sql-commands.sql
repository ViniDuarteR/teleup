...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'status_operador') THEN
    CREATE TYPE status_operador AS ENUM ('Aguardando Chamada', 'Em Chamada', 'Em Pausa', 'Offline');
  END IF;
END$$;
...
CREATE TABLE IF NOT EXISTS operadores (
  ...
  data_criacao     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  data_atualizacao TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  status_operacional status_operador DEFAULT 'Offline'
);
